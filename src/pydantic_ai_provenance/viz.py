...
def to_mermaid(store: ProvenanceStore) -> str:
    """Render the provenance graph as a Mermaid flowchart (LR direction)."""
    graph = store.graph
    lines = ["flowchart LR"]
...
def to_dot(store: ProvenanceStore, graph_name: str = "provenance") -> str:
    ...
    graph = store.graph
    lines = [f'digraph {graph_name} {{', '    rankdir=LR;', '    node [style=filled, fontcolor=white, fontname=Helvetica];']
...
def to_json(store: ProvenanceStore) -> dict[str, Any]:
    """Serialize the provenance graph to a JSON-compatible dict."""
    graph = store.graph

    def _node_dict(n: ProvenanceNode) -> dict[str, Any]:
        return {
            "id": n.id,
            "type": n.type.value,
            "label": n.label,
